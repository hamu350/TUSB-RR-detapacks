...
### 
execute at @e[tag=taisosinnyo] run tag @e[type=#tusb_remake:mob,tag=Enemy,distance=..20] add taisotarget
...
execute at @e[tag=taisotarget] as @a[distance=..10] run scoreboard players add @s HealCount 2
execute at @e[tag=taisotarget] as @a[distance=..10] run function tusb_remake:player/clear_bad_effect
